...
//  boolean critical = FALSE;
  ...
  //  else if (critico)
  //    message( "¡Â¡Le asestas un golpe mortal a%s%s!!\n", enemy->prefix, enemy->name );
  hit = 6;
  enemy->pg -= hit;
}

void
walk (grid_t *grid, int horizontal, int vertical, pj_t *pj, pj_t *enemy) {
  int collision;
...
void
combat_menu(grid_t * combat_grid) {
...
#define COMBAT_MENU_WIDTH  20
  ...
  switch( menu(combat_menu_win, 1, menu_options, MAIN_MENU_PAIR) )
    ...
    case 1: break;
    case 2: break;
    case 3: touchwin(game_win);
      touchwin(stats_win);
      wrefresh(stats_win);
      inventory(combat_grid);
      break;
    default: break;
    }
...
#define MENU_WIN_WIDTH  COLS-10
  ...
  get_focus(help_win);
  mvwprintw(help_win, i=1, (MENU_WIN_WIDTH-20)/2, "Ayuda de Battle Dungeons");
  i++;
  mvwprintw(help_win, ++i, 5, "Movimiento");
  ...
  i++;
  mvwprintw(help_win, ++i, 5, "Acciones");
  mvwprintw(help_win, ++i, 3, "a      -  atacar");
  mvwprintw(help_win,   i, (MENU_WIN_WIDTH-16)/2, "d      -  emborracharse");
  mvwprintw(help_win, ++i, 3, "m      -  magia");
  i++;
  mvwprintw(help_win, ++i, 5, "Misc.");
  mvwprintw(help_win, ++i, 3, "Enter  -  menu de combate");
  mvwprintw(help_win, ++i, 3, "i      -  inventario");

  mvwprintw(help_win, MENU_WIN_HEIGHT-2, MENU_WIN_WIDTH-35, "Presiona cualquier tecla...");
  wgetch(help_win);
  destroy_win(help_win);
}

void
refresh_screen(grid_t *combat_grid, pj_t *pj, pj_t *enemy) {
  int indent = max(strlen(pj->name)+3, 15) + 4;

  werase(message_win);
  werase(stats_win);
  mvwprintw(stats_win, 1, 1,      "%s",             pj->name);
  mvwprintw(stats_win, 2, 1,      "lvl/exp: %i/%i", pj->level, pj->exp );
  mvwprintw(stats_win, 1, indent, "pg: %i(%i)",     pj->pg, pj->pg_max);
  mvwprintw(stats_win, 2, indent, "mp: %i(%i)",     pj->mp, pj->mp_max);
  wrefresh(stats_win);
}

int
pj_round (grid_t *combat_grid, pj_t *pj, pj_t *enemy) {
  bool key_ok;
  ...
  do {
    ...
    switch( wgetch(game_win) )
      {
      case KEY_LEFT:  walk(combat_grid, -1, 0, pj, enemy ); break;
      case '4':       walk(combat_grid, -1, 0, pj, enemy ); break;
      case KEY_RIGHT: walk(combat_grid,  1, 0, pj, enemy ); break;
      case '6':       walk(combat_grid,  1, 0, pj, enemy ); break;
      case KEY_UP:    walk(combat_grid,  0,-1, pj, enemy ); break;
      case '8':       walk(combat_grid,  0,-1, pj, enemy ); break;
      case KEY_DOWN:  walk(combat_grid,  0, 1, pj, enemy ); break;
      case '2':       walk(combat_grid,  0, 1, pj, enemy ); break;
      case '7':       walk(combat_grid, -1,-1, pj, enemy ); break;
      case '9':       walk(combat_grid,  1,-1, pj, enemy ); break;
      case '1':       walk(combat_grid, -1, 1, pj, enemy ); break;
      case '3':       walk(combat_grid,  1, 1, pj, enemy ); break;
      ...
      case 'o':       //object();
	              break;
      case 'i':       inventory(combat_grid);
                      touchwin(game_win);                   break;
	
      case '\n':      combat_menu(combat_grid);
                      touchwin(game_win);                   break;
      case 'h':       help_menu(); touchwin(game_win);      break;
      case 'd':       pj->status.drunk=!pj->status.drunk;   break;
      
      default:        key_ok = FALSE;                       break;
    }
...
int
enemy_round(grid_t *combat_grid, pj_t *pj, pj_t *enemy) {
  ...
  if (grid_node(combat_grid, pj->y+vertical, pj->x)->solid)
    vertical=0;
  if (grid_node(combat_grid, pj->y+vertical, pj->x+horizontal)->solid)
    horizontal=0;

  walk(combat_grid, horizontal, vertical, enemy, pj);
...
void
combate (pj_t *pj, pj_t *enemy) {
  bool pj_turn;
  grid_t * combat_grid;
  int round = 1, temp_round;
...
//  usleep (10000);
  ...
  new_grid(&combat_grid, LINES-7, COLS);
  create_first_combat_grid(combat_grid, pj, enemy);
  keypad(game_win, TRUE);  

  draw_grid( combat_grid, pj, enemy);
  //  pj_turn = ( dices(1, 20, modifier(pj->dex)) > dices(1, 20, modifier(enemy->dex)) );
  pj_turn = TRUE; //

  do {
    
    refresh_screen(combat_grid, pj, enemy); 
    
    if (pj_turn==1) {

      if (pj->status.dead==0)
	pj_turn = pj_round( combat_grid, pj, enemy );
      else
	pj_turn = FALSE;
	...
	pj_turn = enemy_round( combat_grid, pj, enemy );
      else
	pj_turn = TRUE;

    }
    draw_pj(combat_grid, pj);
    draw_pj(combat_grid, enemy);
  ...
  if (pj->pg <= 0)
    {
      ...
      printw( "%s%s cae en combate.", enemy->prefix, enemy->name );
      printw( "¡Â¡Has ganado!!" );
    }
...
new_combat() {
  ...
  mvwprintw( pj_selection_menu_win, 1, 3, "Selecciona Jugador" );
  wattron(pj_selection_menu_win, COLOR_PAIR(50));
  mvwprintw( pj_selection_menu_win, 3, 2, "Nombre :" ); //scanf( "%c", *pj->name );
  mvwprintw( pj_selection_menu_win, 4, 2, "Raza   :" ); //scanf( "%c", *pj->raza );
  ...
  mvwprintw( enemy_selection_menu_win, 1, 3, "Selectiona Oponente:" );
  ...
  option = menu( enemy_selection_menu_win, y+2, menu_options, MAIN_MENU_PAIR );
  {
    switch( option )
      {
      case 1: //buscar_enemigo( enemy );
	break;
      case 2: //enemigo_aleatorio( enemy );
	break;
      default:
	break;
      }
