...
void
move_selection (WINDOW *window, int *option, int direction, int left, int right, int pair) {
...
int
menu (WINDOW *menu_win, int menu_y, char *options[], int color_pair) {

  int n_options = 0;
  int option = 1, key, i, y = menu_y, x, menu_width, longer_option = 0;
...
#define RIGHT (LEFT + strlen(options[longer_option]) + 4)
  ...
  do {
    ...
    switch( key )
      {
      case KEY_DOWN:
	if (option < n_options) move_selection( menu_win, &option,  1, LEFT, RIGHT, color_pair );
	else move_selection( menu_win, &option,  -n_options+1, LEFT, RIGHT, color_pair );
	break;
      case KEY_UP:
	if (option > 1) move_selection( menu_win, &option, -1, LEFT, RIGHT, color_pair );
	else move_selection( menu_win, &option,  n_options-1, LEFT, RIGHT, color_pair );
	break;
      default:
	break;
...
void move_page(WINDOW *window, int *option, int direction, int *temp_len, char *pages[], int pages_init) {
  ...
  if (direction==-2) *temp_len = pages_init + strlen(pages[0]);
  else if (direction==2) for (i=1; i<5; i++) *temp_len += strlen(pages[i]);
  else (*temp_len) += (direction) * ( strlen(pages[(*option)*2+1+direction]) + \
...
#define INVENTORY_WIN_WIDTH 55
  ...
  int option=0, n_pages=(sizeof(inventory_pages)/sizeof(char *)),
    total_len=0, temp_len, pages_init, key, i;  
